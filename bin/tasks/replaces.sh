...
for i in `grep -rl facyl * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;facyl;${THEME_NAME};" $i $i; done
for i in `grep -rl facyl * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;facyl;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;facyl;${THEME_NAME};" $i $i; done
