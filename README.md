## facyl - A minimalist WordPress starter theme

[![Build Status](https://img.shields.io/travis/com/digitoimistodude/facyl.svg?style=flat-square)](https://travis-ci.com/digitoimistodude/facyl) [![GitHub release](https://img.shields.io/github/tag/digitoimistodude/facyl.svg?style=flat-square)](https://github.com/digitoimistodude/facyl/releases) ![GitHub contributors](https://img.shields.io/github/contributors/digitoimistodude/facyl.svg?style=flat-square)

facyl (or simply *Air*) is designed to be a ultra minimal starting point for a WordPress project at [Digitoimisto Dude Oy](https://www.dude.fi), a Finnish boutique digital agency in the center of Jyväskylä. Theme is originally based on [\_s](https://github.com/automattic/_s). We welcome all happy contributors with open arms! [See roadmap](https://favro.com/organization/3b45e73eaf083f68fefef368/c1dd2d4a99d6723904d2e763).

## [Demo](https://airwptheme.com/demo) | [Documentation](https://github.com/digitoimistodude/facyl/wiki)

* **CSS gzipped:** 16.8 KB *(originally 124.4 KB)*
* **JS gzipped:** 8.6 KB *(28.6 KB original)*
* **Front page HTML**: 7.4 KB *(29.4 KB original)*

![facyl-readme-mockup-latest](https://user-images.githubusercontent.com/1534150/170326942-b7404051-7621-4df3-9294-0ccd5e6f59a7.png)

## Weekly updates!

This theme is constantly kept up to date by a bunch of [awesome contributors](https://github.com/digitoimistodude/facyl/graphs/contributors). Wanna join in development? Read [the instructions for contributing](#contributing) and let us know about your first PR!

![Alt](https://repobeats.axiom.co/api/embed/bd95f5747b41d3cf662e033044af08dee9a15f9f.svg "Repobeats analytics image")

### Mission & vision

facyl is built to be very straightforward, backwards compatible, front-end developer friendly and modular by its structure. Following [Underscores](https://github.com/automattic/_s) and [WordPress Theme Coding Standards](https://codex.wordpress.org/Theme_Development#Theme_Development_Standards) best practices and most of the changes in \_s are implemented as soon as they are committed.

**Our mission and goal** is **minimalism** and **simplicity**. **Our vision** is to build a theme that will not implement its own wrappers or functions, will not use any templating languages that would take things further from traditional PHP or CSS, will contain nothing that people will not use or need. facyl will be free of weird "app-like" folder structures or odd syntaxes that nobody else uses. We love WordPress as it was and as it is.

Air was renamed to facyl in version 3.7.8 (March 20th, 2018), because *air* was already taken in the official WordPress theme directory.

### Official, approved, accessibility-ready!

facyl v. 4.2.2 was approved to [official WordPress theme directory](https://wordpress.org/themes/facyl/) on June 4, 2018. But please note, all changes you do to the theme without generating your own or changing textdomain will be overridden in theme updates - so if you use this theme as a starting point, please follow instructions and/or replace the textdomain with your own.
...
Here's the current facyl theme structure (prone to changes, not updating in real time, but stays mostly the same in its pricinpals):

``` shell
themes/your-theme-name/              # → Root of your facyl based theme
├── 404.php                          # → Default "not found" page
├── archive.php                      # → Default archive template
├── bin/                             # → Scripts
│   ├── ...                          # → This scripts are used for generating themes or new facyl version
│   └── newtheme.sh                  # → The start script for creating YOUR own theme out of facyl
...
│   └── ...                          #
├── gulpfile.js                      # → Core gulpfile for facyl development
...
facyl can register your CPT:s automatically.
...
facyl can register your Taxonomies automatically.
...
facyl uses namespaced PHP since 5.0.0. This means that we no longer need to prefix functions and hooks, because `namespace Air_Light;` takes care of that.
...
Please refer to Wiki section [Getting started in theming with facyl](https://github.com/digitoimistodude/facyl/wiki#2-getting-started-in-theming-with-facyl).

### Usage

See [Documentation](https://github.com/digitoimistodude/facyl/wiki).
...
5. Create a fork of facyl (press fork button on GitHub) (or if you are Dude staff, just create new branch for changes)
6. Go to the theme folder of your WordPress instance via Terminal (`cd /var/www/airdev/content/themes`)
7. Clone your fork with `git clone git@github.com:yourusername/facyl.git` (replace yourusername with your actual username)
8. Cd to your new cloned repository `cd /var/www/airdev/content/themes/facyl`
...
11. Activate theme - if you are using the lightweight [macos-lemp-setup](https://github.com/digitoimistodude/macos-lemp-setup): `cd /var/www/airdev && vendor/wp-cli/wp-cli/bin/wp theme activate facyl`
12. Open whole project to your preferred coding editor, for example when using [Visual Studio Code](https://github.com/ronilaukkarinen/vscode-settings) that would be `code /var/www/airdev/content/themes/facyl` or via GUI (Open folder).
13. Go to back to facyl dir with `cd /var/www/airdev/content/themes/facyl` and then run `gulp` and start developing! Please note, contributing to this theme is only possible when gulp is run from theme directory, NOT on project root.
...
To install facyl to your own development environment, just clone your fork to your theme directory, activate the theme, and make changes. If you make changes to front-end (JS/SCSS), you'll need to use our gulpfile and npm dependencies, so make sure you go through steps 9-10 and 12 above.
...
- [GitHub](https://github.com/digitoimistodude/facyl)
- [WordPress.org](https://wordpress.org/themes/facyl/)
...
``` bash
alias release_new_air_version='git push && git push --tags && rsync -av -e ssh --exclude={"/node_modules/*","/bin/*","/sass/*"} /var/www/airdev/content/themes/facyl/* YOURUSERNAME@YOURSERVER:/var/www/airwptheme.com/public_html/demo/content/themes/facyl/ && /var/www/airdev/content/themes/facyl/bin && sh air-move-out.sh && sh air-pack.sh'
```
...
**Theme developers please note:** if you use phpcs in [SublimeLinter as custom standard](https://github.com/ronilaukkarinen/sublime-settings/blob/master/Library/Application%20Support/Sublime%20Text%203/Packages/User/SublimeLinter.sublime-settings#L47) on [dudestack](https://github.com/digitoimistodude/dudestack), you will need extra content/themes/facyl subfolders inside theme directory for it to work on both global projects and with facyl.

### Known issues

See tool related issues in [devpackages](https://github.com/digitoimistodude/devpackages#known-issues) and [facyl issue tracker](https://github.com/digitoimistodude/issues).
